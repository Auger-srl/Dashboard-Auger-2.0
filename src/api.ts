import { Article, Process, Commitment, Client, MovementLog, User, ChatMessage, Macchina5000, FaseTaglio, TaglioLaser, PiastraAT, PortaAT, InvolucroAT, TimedProductionItem } from './types';

// Helper for API calls with exponential backoff retry
export async function apiCall<T>(url: string, options?: RequestInit, retries = 5, backoff = 500): Promise<T> {
  ...
  try {
    ...
    if (response.status === 429 && retries > 0) {
      ...
      console.warn(`Rate limit exceeded for ${url} (Status 429). Retrying in ${backoff}ms... (${retries} retries left)`);
    ...
    // Special platform handling: "Rate exceeded" can sometimes be in the body text
    if (text.includes('Rate exceeded') && retries > 0) {
      clearTimeout(timeoutId);
      console.warn(`Rate limit body text detected for ${url}. Retrying in ${backoff}ms... (${retries} retries left)`);
      await new Promise(resolve => setTimeout(resolve, backoff));
      return apiCall<T>(url, options, retries - 1, backoff * 2);
    }
    ...
    if (contentType && !contentType.includes('application/json')) {
      ...
      // If it's not JSON but response is OK, check if it's an error message disguised as OK
      if (text.toLowerCase().includes('rate limit') || text.includes('Rate exceeded')) {
...
export const fetchDashboardData = async (): Promise<{
  ...
  fasePannelatrice: TimedProductionItem[],
  fasePiegaManuale: TimedProductionItem[],
  faseVerniciatura: any[],
...
export const addCommitmentBatch = async (data: any) => {
  return apiCall('/api/commitments/batch', {
    method: 'POST',
...
export const addFaseSaldatura = async (data: any) => {
...
export const addFasePannelatrice = async (data: any) => {
  return apiCall('/api/fase-pannelatrice', {
    method: 'POST',
    body: JSON.stringify(data),
  });
};

export const addFasePiegaManuale = async (data: any) => {
  return apiCall('/api/fase-piega-manuale', {
    method: 'POST',
    body: JSON.stringify(data),
  });
};

export const addFaseVerniciatura = async (data: any) => {
  return apiCall('/api/fase-verniciatura', {
    method: 'POST',
    body: JSON.stringify(data),
  });
};
...
// Generic Timed Phase Functions
export const fetchTimedPhase = async <T>(apiPath: string): Promise<T[]> => {
  return apiCall<T[]>(apiPath);
};

export const addTimedPhase = async <T>(apiPath: string, data: Omit<T, 'id'>) => {
  return apiCall(apiPath, {
    method: 'POST',
    body: JSON.stringify(data),
  });
};

export const updateTimedPhase = async <T>(apiPath: string, id: string, data: Partial<T>) => {
  return apiCall(`${apiPath}/${id}`, {
    method: 'PUT',
    body: JSON.stringify(data),
  });
};

export const deleteTimedPhase = async (apiPath: string, id: string) => {
  return apiCall(`${apiPath}/${id}`, {
    method: 'DELETE',
  });
};
