...
import { fetchTaglioLaser, addTaglioLaser, updateTaglioLaser, deleteTaglioLaser, addMovementLog, addMovimentoCGialla, addFasePannelatrice, addFaseSaldatura, addFaseVerniciatura } from '../api';
...
export default function TaglioLaser({ articles, username, role, onUpdate }: TaglioLaserProps) {
  ...
  const handleComplete = async (item: TaglioLaserType, prep: number, fine: string, confirmedQty: number) => {
    try {
      ...
      // Update Taglio Laser
      const updateData: Partial<TaglioLaserType> = {
        preparazione: prep,
        fine,
      ...
      await updateTaglioLaser(item.id, updateData);
