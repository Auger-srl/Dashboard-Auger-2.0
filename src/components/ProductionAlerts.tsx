...
export default function ProductionAlerts({ username, alerts: propAlerts, onDismiss }: ProductionAlertsProps) {
  const [internalAlerts, setInternalAlerts] = useState<ProductionAlert[]>([]);
  const isAuthorized = ['LucaTurati', 'RobertoBonalumi', 'TahaDev'].includes(username);

  // Use prop alerts if provided, otherwise fallback to internal state (for backward compatibility if needed)
  const alerts = propAlerts !== undefined ? propAlerts : internalAlerts;

  const fetchAlerts = async () => {
    if (propAlerts !== undefined) return; // Don't fetch if we have props
    try {
