...
import { Article, Process, Commitment, Macchina5000, TaglioLaser, FaseTaglio, TimedProductionItem } from '../types';
import { getDisponibilita, getCategory, isPhaseEnabled } from '../utils';
import { Package, Search, X, Filter, ChevronDown, Menu, Scissors, Flame, Play, Plus, Edit2, CheckCircle, Loader2, CornerDownRight, PaintBucket, Activity } from 'lucide-react';
import clsx from 'clsx';
import React from 'react';
import { addMacchina5000, addTaglioLaser, addFaseTaglio, addFaseSaldatura, addFasePannelatrice, addFasePiegaManuale, addFaseVerniciatura, updateArticle, updateProcess, addArticle, apiCall } from '../api';
...
interface Produzione2026ViewProps {
  ...
  faseSaldatura?: TimedProductionItem[];
  onUpdate: () => void;
...
export default function Produzione2026View({ 
  ...
  const [selectedArticleForPhase, setSelectedArticleForPhase] = useState<Article | null>(null);
  const [autoPhaseForm, setAutoPhaseForm] = useState({
    quantita: 0,
    odl: '',
    cliente: 'MAGAZZINO',
    commessa: 'PRODUZIONE 2026',
    macchina: 'Macchina 5000' as string,
    fase: 'Taglio' as 'Taglio' | 'Piega' | 'Saldatura' | 'Verniciatura'
  });
  ...
  const handleAutoPhase = async () => {
    if (!selectedArticleForPhase) return;
    try {
      const { fase, cliente, quantita, odl, commessa, macchina } = autoPhaseForm;
      const data = {
        lavorazione_per: cliente,
        articolo: selectedArticleForPhase.nome,
        quantita: quantita,
      ...
      if (fase === 'Taglio') {
        await addFaseTaglio(data);
        toast.success(`Articolo ${selectedArticleForPhase.nome} inviato a RidaTecnico per ${macchina}`);
      } else if (fase === 'Saldatura') {
        await addFaseSaldatura(data);
        toast.success(`Articolo ${selectedArticleForPhase.nome} inviato al ${macchina}`);
      } else if (fase === 'Piega') {
        if (macchina === 'Pannellatrice') {
          await addFasePannelatrice(data);
        } else {
          await addFasePiegaManuale(data);
        }
        toast.success(`Articolo ${selectedArticleForPhase.nome} inviato a ${macchina}`);
      } else if (fase === 'Verniciatura') {
        await addFaseVerniciatura(data);
        toast.success(`Articolo ${selectedArticleForPhase.nome} inviato in Verniciatura`);
      }
  ...
  const openAutoPhaseModal = (article: Article, phase: 'Taglio' | 'Piega' | 'Saldatura' | 'Verniciatura' = 'Taglio') => {
    const totalImpCount = getArticleTotalCommitments(article.id, phase);
    const availableVal = phase === 'Verniciatura' ? (article.verniciati || 0) : 0; // Simplified for display
    const disp = availableVal - totalImpCount;
    const process = getProcess(article.id);
    ...
    setSelectedArticleForPhase(article);
    
    let defaultMacchina = 'Macchina 5000';
    if (phase === 'Piega') defaultMacchina = 'Pannellatrice';
    if (phase === 'Saldatura') defaultMacchina = 'Saldatrice Manuale';
    if (phase === 'Verniciatura') defaultMacchina = 'Verniciatura';
    if (phase === 'Taglio' && isOsvaldo && !isAndrea && !isLucaTurati && !isDeveloper) defaultMacchina = 'Taglio Laser';
  ...
  return (
    <div className="flex flex-col gap-6 h-full">
        ...
        >
          <div ref={contentRef} className="flex gap-6 h-full items-start px-1 w-max">
            {columns.map((col, colIdx) => {
              ...
              return (
                <div key={colIdx} className="flex flex-col gap-6 min-w-[600px] w-[600px] flex-shrink-0">
                  {visibleCategories.map(category => {
                    let categoryArticles = filteredArticles.filter(a => getCategory(a.nome || '', a.codice || '') === category);
                    
                    const autoButtonsCount = [
                      isPhaseEnabled(category, 'Taglio'),
                      isPhaseEnabled(category, 'Piega'),
                      isPhaseEnabled(category, 'Saldatura') || category === 'Strutture Agr',
                      isPhaseEnabled(category, 'Verniciatura')
                    ].filter(Boolean).length;
                    
                    const autoColClass = autoButtonsCount <= 1 ? "w-8" : 
                                         autoButtonsCount === 2 ? "w-16" :
                                         autoButtonsCount === 3 ? "w-24" : "w-32";
                    ...
                    <table className="w-full text-left border-collapse text-sm">
                      <thead className="bg-slate-100 text-slate-600 text-[10px] uppercase tracking-wider">
                        <tr>
                          ...
                          {isDeveloper && (
                            <th className="px-2 py-2 font-semibold text-center border-b border-slate-300 border-r w-12">Azioni</th>
                          )}
                          {canAutoCut && (
                            <th className={clsx("px-2 py-2 font-semibold text-center border-b border-slate-300 border-r", autoColClass)} title="Invio Produzione Rapido">Auto</th>
                          )}
                          {isPhaseEnabled(category, 'Taglio') && (
                      ...
                      <tbody className="bg-white divide-y divide-slate-200">
                        {categoryArticles.length === 0 && (
                          <tr>
                            <td colSpan={2 + (isDeveloper ? 1 : 0) + (canAutoCut ? 1 : 0) + (isPhaseEnabled(category, 'Taglio') ? 1 : 0) + (!isRestricted ? (3 + [isPhaseEnabled(category, 'Piega'), isPhaseEnabled(category, 'Saldatura'), isPhaseEnabled(category, 'Verniciatura')].filter(Boolean).length) : 0)} className="px-3 py-8 text-center text-slate-500 italic">
                        ...
                        {categoryArticles.map(article => {
                          ...
                          const isSmallPorte = ['Porte Standard', 'Porte IB/CB'].includes(category);
                          ...
                          return (
                            <tr key={article.id} className={clsx("hover:bg-slate-50 transition-colors h-10 group/row", isEditing && "bg-blue-50", isSmallPorte && "h-8")}>
                              <td className={clsx("px-3 py-0.5 border-r border-slate-200", articleNameClass)}>
                                {isEditing ? (
                              ...
                              {canAutoCut && (
                                <td className="px-1 py-1 border-r border-slate-200 text-center">
                                  <div className="flex items-center justify-center gap-1 mx-auto whitespace-nowrap">
                                    ...
                                    {isPhaseEnabled(category, 'Taglio') && (
                                      <button
                                        onClick={() => canUseAutoCut && openAutoPhaseModal(article, 'Taglio')}
                                        disabled={!canUseAutoCut}
                                        className={clsx(
                                          isSmallPorte ? "p-1 rounded-md" : "p-1.5 rounded-lg",
                                          "transition-all shadow-sm",
                                          canUseAutoCut ? "hover:scale-110 active:scale-95 cursor-pointer bg-blue-100 text-blue-700 hover:bg-blue-200 border border-blue-300" : "opacity-50 cursor-not-allowed bg-slate-100 text-slate-400"
                                        )}
                                        title="Invia alla Macchina 5000/Taglio Laser"
                                      >
                                        <Scissors className={isSmallPorte ? "w-3 h-3" : "w-3.5 h-3.5"} />
                                      </button>
                                    )}
                                    ...
                                    {isPhaseEnabled(category, 'Piega') && (
                                      <button
                                        onClick={() => canUseAutoCut && openAutoPhaseModal(article, 'Piega')}
                                        disabled={!canUseAutoCut}
                                        className={clsx(
                                          isSmallPorte ? "p-1 rounded-md" : "p-1.5 rounded-lg",
                                          "transition-all shadow-sm",
                                          canUseAutoCut ? "hover:scale-110 active:scale-95 cursor-pointer bg-emerald-100 text-emerald-700 hover:bg-emerald-200 border border-emerald-300" : "opacity-50 cursor-not-allowed bg-slate-100 text-slate-400"
                                        )}
                                        title="Invia alla Piegatrice (Pannellatrice/Manuale)"
                                      >
                                        <CornerDownRight className={isSmallPorte ? "w-3 h-3" : "w-3.5 h-3.5"} />
                                      </button>
                                    )}
                                    ...
                                    {(isPhaseEnabled(category, 'Saldatura') || category === 'Strutture Agr') && (
                                      <button
                                        onClick={() => canUseAutoCut && openAutoPhaseModal(article, 'Saldatura')}
                                        disabled={!canUseAutoCut}
                                        className={clsx(
                                          isSmallPorte ? "p-1 rounded-md" : "p-1.5 rounded-lg",
                                          "transition-all shadow-sm",
                                          canUseAutoCut ? "hover:scale-110 active:scale-95 cursor-pointer bg-red-100 text-red-700 hover:bg-red-200 border border-red-300" : "opacity-50 cursor-not-allowed bg-slate-100 text-slate-400"
                                        )}
                                        title="Invia al Reparto Saldatura"
                                      >
                                        <Flame className={isSmallPorte ? "w-3 h-3" : "w-3.5 h-3.5"} />
                                      </button>
                                    )}
                                    ...
                                    {isPhaseEnabled(category, 'Verniciatura') && (
                                      <button
                                        onClick={() => canUseAutoCut && openAutoPhaseModal(article, 'Verniciatura')}
                                        disabled={!canUseAutoCut}
                                        className={clsx(
                                          isSmallPorte ? "p-1 rounded-md" : "p-1.5 rounded-lg",
                                          "transition-all shadow-sm",
                                          canUseAutoCut ? "hover:scale-110 active:scale-95 cursor-pointer bg-purple-100 text-purple-700 hover:bg-purple-200 border border-purple-300" : "opacity-50 cursor-not-allowed bg-slate-100 text-slate-400"
                                        )}
                                        title="Invia in Verniciatura"
                                      >
                                        <PaintBucket className={isSmallPorte ? "w-3 h-3" : "w-3.5 h-3.5"} />
                                      </button>
                                    )}
                                  </div>
                                </td>
                              ...
                              {!isRestricted && (
                                <>
                                  ...
                                  {isPhaseEnabled(category, 'Verniciatura') && renderPhaseCell(article.verniciati, 'Verniciatura', article.id, 'bg-purple-50/30 border-r border-slate-200', category)}
                                  <td 
                                    className={clsx(
      ...
      {/* Modal for Auto Phase Request */}
      {isPhaseModalOpen && selectedArticleForPhase && (
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200 animate-in zoom-in-95 duration-200">
            <div className="bg-slate-800 px-6 py-4 flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div className={clsx(
                  ...
                  autoPhaseForm.fase === 'Taglio' ? "bg-blue-500" : 
                  autoPhaseForm.fase === 'Piega' ? "bg-emerald-500" :
                  autoPhaseForm.fase === 'Saldatura' ? "bg-red-500" : "bg-purple-500"
                )}>
                  {autoPhaseForm.fase === 'Taglio' ? <Scissors className="w-5 h-5" /> : 
                   autoPhaseForm.fase === 'Piega' ? <CornerDownRight className="w-5 h-5" /> :
                   autoPhaseForm.fase === 'Saldatura' ? <Flame className="w-5 h-5" /> : <PaintBucket className="w-5 h-5" />}
                </div>
                <div>
                  <h3 className="text-white font-bold">Invia a Reparto {autoPhaseForm.fase}</h3>
                  <p className="text-slate-400 text-[10px] uppercase tracking-wider">Automazione Produzione 2026</p>
                </div>
              </div>
              <button onClick={() => setIsPhaseModalOpen(false)} className="text-slate-400 hover:text-white transition-colors">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 mb-4">
                ...
                <div className="font-bold text-slate-800">{selectedArticleForPhase.nome}</div>
                <div className="text-xs text-slate-500 font-mono">{selectedArticleForPhase.codice}</div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-700 uppercase">Quantità da Lavorare</label>
                  <input
                    type="number"
                    value={autoPhaseForm.quantita}
                    onChange={(e) => setAutoPhaseForm({ ...autoPhaseForm, quantita: parseInt(e.target.value) || 0 })}
                    className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl focus:ring-2 focus:ring-slate-900 outline-none font-mono font-bold text-lg"
                    autoFocus
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <input
                    type="text"
                    value={autoPhaseForm.odl}
                    onChange={(e) => setAutoPhaseForm({ ...autoPhaseForm, odl: e.target.value })}
                    className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl focus:ring-2 focus:ring-slate-900 outline-none"
              ...
              <div className="space-y-1.5">
                ...
                <input
                  type="text"
                  value={autoPhaseForm.cliente}
                  onChange={(e) => setAutoPhaseForm({ ...autoPhaseForm, cliente: e.target.value })}
                  className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl focus:ring-2 focus:ring-slate-900 outline-none font-medium"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <input
                  type="text"
                  value={autoPhaseForm.commessa}
                  onChange={(e) => setAutoPhaseForm({ ...autoPhaseForm, commessa: e.target.value })}
                  className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl focus:ring-2 focus:ring-slate-900 outline-none font-medium"
                />
              </div>

              {autoPhaseForm.fase === 'Taglio' && (
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-700 uppercase">Seleziona Macchina</label>
                  <select
                    value={autoPhaseForm.macchina}
                    onChange={(e) => setAutoPhaseForm({ ...autoPhaseForm, macchina: e.target.value })}
                    className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl focus:ring-2 focus:ring-slate-900 outline-none font-medium"
                  >
              ...
              {autoPhaseForm.fase === 'Piega' && (
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl focus:ring-2 focus:ring-slate-900 outline-none font-medium"
                  >
                    <option value="Pannellatrice">Pannellatrice</option>
                    <option value="Piega Manuale">Piega Manuale</option>
                  </select>
                </div>
              )}

              {autoPhaseForm.fase === 'Saldatura' && (
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-700 uppercase">Tipo Saldatura</label>
                  <select
                    value={autoPhaseForm.macchina}
                    onChange={(e) => setAutoPhaseForm({ ...autoPhaseForm, macchina: e.target.value })}
                    className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl focus:ring-2 focus:ring-slate-900 outline-none font-medium"
                  >
                    <option value="Saldatrice Manuale">Saldatrice Manuale</option>
                    <option value="Sald. Auto STT-STB & Piantane">Sald. Auto STT-STB & Piantane</option>
                    <option value="Sald Auto Casse">Sald Auto Casse</option>
                    <option value="Sald Auto AGR">Sald Auto AGR</option>
                  </select>
                </div>
              )}
            </div>
            ...
            <div className="p-6 pt-0 flex gap-3">
              ...
              <button
                ...
                disabled={autoPhaseForm.quantita <= 0}
                className={clsx(
                  "flex-1 px-4 py-2.5 text-white font-bold rounded-xl transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2",
                  autoPhaseForm.fase === 'Taglio' ? "bg-blue-500 hover:bg-blue-600 shadow-blue-500/20" : 
                  autoPhaseForm.fase === 'Piega' ? "bg-emerald-500 hover:bg-emerald-600 shadow-emerald-500/20" :
                  autoPhaseForm.fase === 'Saldatura' ? "bg-red-500 hover:bg-red-600 shadow-red-500/20" : "bg-purple-500 hover:bg-purple-600 shadow-purple-500/20"
                )}
              >
