...
import { LogOut, LayoutDashboard, Menu, Package, Activity, Users, History, BellRing, Grid, ChevronDown, ChevronRight, MonitorPlay, Scissors, RefreshCw, PaintBucket, Flame, Sparkles, Database } from 'lucide-react';
...
import GenericFaseView from './GenericFaseView';
import Pannelatrice from './Pannelatrice';
import { Article, Process, Commitment, Client, AUTHORIZED_USERS, Macchina5000 as Macchina5000Type, TaglioLaser as TaglioLaserType, FaseTaglio, TimedProductionItem } from '../types';
import { apiCall, fetchDashboardData } from '../api';
...
export default function Dashboard({ username, role, onLogout }: DashboardProps) {
  ...
  const [faseSaldatura, setFaseSaldatura] = useState<TimedProductionItem[]>([]);
  const [fasePannelatrice, setFasePannelatrice] = useState<TimedProductionItem[]>([]);
  const [fasePiegaManuale, setFasePiegaManuale] = useState<TimedProductionItem[]>([]);
  const [faseVerniciatura, setFaseVerniciatura] = useState<any[]>([]);
  ...
  const [isTaglioMenuExpanded, setIsTaglioMenuExpanded] = useState(false);
  const [isSaldaturaMenuExpanded, setIsSaldaturaMenuExpanded] = useState(false);
  const [isPiegaMenuExpanded, setIsPiegaMenuExpanded] = useState(false);
  ...
  const fetchData = async (isManual = false) => {
    ...
    try {
      ...
      const { 
        ...
        macchina5000: macchina5000Data, 
        taglioLaser: taglioLaserData, 
        faseTaglio: faseTaglioData, 
        faseSaldatura: faseSaldaturaData, 
        fasePannelatrice: fasePannelatriceData,
        fasePiegaManuale: fasePiegaManualeData,
        faseVerniciatura: faseVerniciaturaData,
        productionAlerts: productionAlertsData
      } = data;
      ...
      if (!Array.isArray(articlesData) || !Array.isArray(processesData) || !Array.isArray(commitmentsData) || !Array.isArray(macchina5000Data) || !Array.isArray(taglioLaserData) || !Array.isArray(faseTaglioData) || !Array.isArray(faseSaldaturaData) || !Array.isArray(faseVerniciaturaData)) {
        console.error('Data received is not in expected format', { articlesData, processesData, commitmentsData, macchina5000Data, taglioLaserData, faseTaglioData, faseSaldaturaData, faseVerniciaturaData });
      ...
      setFasePannelatrice(fasePannelatriceData);
      setFasePiegaManuale(fasePiegaManualeData);
      setFaseVerniciatura(faseVerniciaturaData);
      setProductionAlerts(productionAlertsData);
  ...
  const getViewTitle = () => {
    switch (currentView) {
      ...
      case 'schedaVerniciatura': return 'Scheda Verniciatura';
      case 'sald-auto-stt-stb-piantane': return 'Sald. Auto STT-STB & Piantane';
      case 'sald-auto-casse': return 'Sald. Auto Casse';
      case 'sald-auto-agr': return 'Sald. Auto AGR';
      case 'sald-manuale': return 'Saldatrice Manuale';
      case 'fasePannelatrice': return 'Pannellatrice';
      case 'fasePiegaManuale': return 'Piega Manuale';
      default: return '';
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans relative">
      ...
      <ProductionAlerts 
        ...
        onDismiss={(id) => setProductionAlerts(prev => prev.filter(a => a.id !== id))}
      />
          ...
          <div className="absolute top-20 left-4 w-72 bg-slate-900/95 backdrop-blur-xl shadow-2xl border border-slate-700/50 py-3 rounded-2xl z-50 overflow-y-auto max-h-[calc(100vh-120px)] custom-scrollbar">
            ...
            {((role !== 'taglio_only' && !isElena && !isRida) || (role === 'admin' && !isElena) || role === 'developer') && (
              <>
                ...
                {(!isAndrea && !isOsvaldo && !isRida) || role === 'developer' ? (
                  <button
                    onClick={() => { setCurrentView('schedaVerniciatura'); setMenuOpen(false); }}
                    className={`w-full text-left px-6 py-3.5 flex items-center gap-3 transition-all ${currentView === 'schedaVerniciatura' ? 'bg-orange-500/10 text-orange-400 border-l-4 border-orange-500 font-semibold' : 'text-slate-300 hover:bg-white/5 hover:text-white border-l-4 border-transparent font-medium'}`}
                  >
                    <PaintBucket className="h-5 w-5" />
                    <span>Scheda Vern.</span>
                  </button>
                ) : null}
              </>
            )}
            ...
            {!isElena && isTaglioVisible && (
              <div className="flex flex-col">
                <div className="flex items-center">
                  <button
                    onClick={() => { setIsTaglioMenuExpanded(!isTaglioMenuExpanded); }}
                    className={`flex-1 text-left px-6 py-3.5 flex items-center gap-3 transition-all ${['faseTaglio', 'macchina5000', 'taglioLaser'].includes(currentView) ? 'bg-rose-500/10 text-rose-400 border-l-4 border-rose-500 font-semibold' : 'text-slate-300 hover:bg-white/5 hover:text-white border-l-4 border-transparent font-medium'}`}
                  >
                    <Activity className="h-5 w-5" />
                    <span>Fase Taglio</span>
                  </button>
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
                      setIsTaglioMenuExpanded(!isTaglioMenuExpanded);
                    }}
                    className="px-4 py-3.5 text-slate-400 hover:text-white hover:bg-white/5 transition-colors border-l border-slate-700/50"
                  >
                    {isTaglioMenuExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                  </button>
                </div>
                {isTaglioMenuExpanded && (
                  <div className="bg-slate-900/50 py-1 border-l-4 border-transparent">
                    {!isOsvaldo && (
                      <button
                        onClick={() => { setCurrentView('macchina5000'); setMenuOpen(false); }}
                        className={`w-full text-left pl-14 pr-6 py-2 text-xs transition-colors ${currentView === 'macchina5000' ? 'text-rose-400 font-bold' : 'text-slate-400 hover:text-white'}`}
                      >
                        Macchina 5000
                      </button>
                    )}
                    {!isAndrea && (
                      <button
                        ...
                        className={`w-full text-left pl-14 pr-6 py-2 text-xs transition-colors ${currentView === 'taglioLaser' ? 'text-rose-400 font-bold' : 'text-slate-400 hover:text-white'}`}
                      >
                        Taglio Laser
                      </button>
                    )}
            ...
            {!isElena && isTaglioVisible && (
              <div className="flex flex-col">
                <div className="flex items-center">
                  <button
                    onClick={() => { setIsPiegaMenuExpanded(!isPiegaMenuExpanded); }}
                    className={`flex-1 text-left px-6 py-3.5 flex items-center gap-3 transition-all ${['fasePannelatrice', 'fasePiegaManuale'].includes(currentView) ? 'bg-amber-500/10 text-amber-400 border-l-4 border-amber-500 font-semibold' : 'text-slate-300 hover:bg-white/5 hover:text-white border-l-4 border-transparent font-medium'}`}
                  >
                    <Activity className="h-5 w-5" />
                    <span>Macchine Piegatura</span>
                  </button>
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
                      setIsPiegaMenuExpanded(!isPiegaMenuExpanded);
                    }}
                    className="px-4 py-3.5 text-slate-400 hover:text-white hover:bg-white/5 transition-colors border-l border-slate-700/50"
                  >
                    {isPiegaMenuExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                  </button>
                </div>
                {isPiegaMenuExpanded && (
                  <div className="bg-slate-900/50 py-1 border-l-4 border-transparent">
                    <button
                      onClick={() => { setCurrentView('fasePannelatrice'); setMenuOpen(false); }}
                      className={`w-full text-left pl-14 pr-6 py-2 text-xs transition-colors ${currentView === 'fasePannelatrice' ? 'text-amber-400 font-bold' : 'text-slate-400 hover:text-white'}`}
                    >
                      Pannellatrice
                    </button>
                    <button
                      onClick={() => { setCurrentView('fasePiegaManuale'); setMenuOpen(false); }}
                      className={`w-full text-left pl-14 pr-6 py-2 text-xs transition-colors ${currentView === 'fasePiegaManuale' ? 'text-amber-400 font-bold' : 'text-slate-400 hover:text-white'}`}
                    >
                      Piega Manuale
                    </button>
                  </div>
            ...
            {!isElena && isTaglioVisible && (
              <div className="flex flex-col">
                <div className="flex items-center">
                  <button
                    onClick={() => { setIsSaldaturaMenuExpanded(!isSaldaturaMenuExpanded); }}
                    className={`flex-1 text-left px-6 py-3.5 flex items-center gap-3 transition-all ${['sald-manuale', 'sald-auto-stt-stb-piantane', 'sald-auto-casse', 'sald-auto-agr'].includes(currentView) ? 'bg-orange-500/10 text-orange-400 border-l-4 border-orange-500 font-semibold' : 'text-slate-300 hover:bg-white/5 hover:text-white border-l-4 border-transparent font-medium'}`}
                  >
                    <Flame className="h-5 w-5" />
                    <span>Macchine Saldatrici</span>
                  </button>
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
                      setIsSaldaturaMenuExpanded(!isSaldaturaMenuExpanded);
                    }}
                    className="px-4 py-3.5 text-slate-400 hover:text-white hover:bg-white/5 transition-colors border-l border-slate-700/50"
                  >
                    {isSaldaturaMenuExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                  </button>
                </div>
                {isSaldaturaMenuExpanded && (
                  <div className="bg-slate-900/50 py-1 border-l-4 border-transparent">
                    <button
                      onClick={() => { setCurrentView('sald-manuale'); setMenuOpen(false); }}
                      className={`w-full text-left pl-14 pr-6 py-2 text-xs transition-colors ${currentView === 'sald-manuale' ? 'text-orange-400 font-bold' : 'text-slate-400 hover:text-white'}`}
                    >
                      Saldatrice Manuale
                    </button>
                    <button
                      onClick={() => { setCurrentView('sald-auto-stt-stb-piantane'); setMenuOpen(false); }}
                      className={`w-full text-left pl-14 pr-6 py-2 text-xs transition-colors ${currentView === 'sald-auto-stt-stb-piantane' ? 'text-orange-400 font-bold' : 'text-slate-400 hover:text-white'}`}
                    >
                      Sald. STT/STB & Piantane
                    </button>
                    <button
                      onClick={() => { setCurrentView('sald-auto-casse'); setMenuOpen(false); }}
                      className={`w-full text-left pl-14 pr-6 py-2 text-xs transition-colors ${currentView === 'sald-auto-casse' ? 'text-orange-400 font-bold' : 'text-slate-400 hover:text-white'}`}
                    >
                      Sald Auto Casse
                    </button>
                    <button
                      onClick={() => { setCurrentView('sald-auto-agr'); setMenuOpen(false); }}
                      className={`w-full text-left pl-14 pr-6 py-2 text-xs transition-colors ${currentView === 'sald-auto-agr' ? 'text-orange-400 font-bold' : 'text-slate-400 hover:text-white'}`}
                    >
                      Sald Auto AGR
                    </button>
                  </div>
            ...
            {!isElena && isTaglioVisible && (
              <button
                onClick={() => { setCurrentView('faseVerniciatura'); setMenuOpen(false); }}
                className={`w-full text-left px-6 py-3.5 flex items-center gap-3 transition-all ${currentView === 'faseVerniciatura' ? 'bg-purple-500/10 text-purple-400 border-l-4 border-purple-500 font-semibold' : 'text-slate-300 hover:bg-white/5 hover:text-white border-l-4 border-transparent font-medium'}`}
              >
                <PaintBucket className="h-5 w-5" />
                <span>Fase Verniciatura</span>
              </button>
            )}
      ...
      <main className="flex-1 max-w-[1600px] w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col gap-8 relative z-10" onClick={() => menuOpen && setMenuOpen(false)}>
        {loading ? (
          ...
          <Pannelatrice
            ...
            role={role}
            onUpdate={handleArticleUpdate}
            apiPath="/api/fase-saldatura"
            title="Saldatura Automatica STT-STB & Piantane"
            machineName="Sald. Auto STT-STB & Piantane"
            filterMachine="Sald. Auto STT-STB & Piantane"
            faseLogName="Saldatura Automatica STT/STB"
            faseCGiallaName="Saldatura"
            items={faseSaldatura}
            articleFilter={(a) => {
              const upperNome = (a.nome || '').toUpperCase();
              const upperCodice = (a.codice || '').toUpperCase();
              return upperCodice.includes('STT') || 
                     upperCodice.includes('STB') || 
                     upperNome.includes('STRUTTURA BASE') || 
                     upperNome.includes('STRUTTURA TETTO') ||
                     upperNome.includes('STRUTTURE BASI') ||
                     upperNome.includes('STRUTTURE TETTI') ||
                     upperNome.includes('STRUTTURE A TETTO');
            }}
          />
        ) : currentView === 'sald-auto-casse' ? (
          <Pannelatrice
            articles={articles}
            username={username}
            role={role}
            onUpdate={handleArticleUpdate}
            apiPath="/api/fase-saldatura"
            title="Saldatura Automatica Casse"
            machineName="Sald Auto Casse"
            filterMachine="Sald Auto Casse"
            faseLogName="Saldatura Automatica Casse"
            faseCGiallaName="Saldatura"
            items={faseSaldatura}
          />
        ) : currentView === 'sald-auto-agr' ? (
          <Pannelatrice
            articles={articles}
            username={username}
            role={role}
            onUpdate={handleArticleUpdate}
            apiPath="/api/fase-saldatura"
            title="Saldatura Automatica AGR"
            machineName="Sald Auto AGR"
            filterMachine="Sald Auto AGR"
            faseLogName="Saldatura Automatica AGR"
            faseCGiallaName="Saldatura"
            items={faseSaldatura}
          />
        ) : currentView === 'sald-manuale' ? (
          <Pannelatrice
            articles={articles}
            username={username}
            role={role}
            onUpdate={handleArticleUpdate}
            apiPath="/api/fase-saldatura"
            title="Saldatrice Manuale"
            machineName="Saldatrice Manuale"
            filterMachine="Saldatrice Manuale"
            faseLogName="Saldatura Manuale"
            faseCGiallaName="Saldatura"
            items={faseSaldatura}
          />
        ) : currentView === 'fasePannelatrice' ? (
          <Pannelatrice
            articles={articles}
            username={username}
            role={role}
            onUpdate={handleArticleUpdate}
            apiPath="/api/fase-pannelatrice"
            title="Pannellatrice"
            machineName="Pannellatrice"
            faseLogName="Pannellatura"
            faseCGiallaName="Piega"
            items={fasePannelatrice}
          />
        ) : currentView === 'fasePiegaManuale' ? (
          <Pannelatrice
            articles={articles}
            username={username}
            role={role}
            onUpdate={handleArticleUpdate}
            apiPath="/api/fase-piega-manuale"
            title="Piega Manuale"
            machineName="Piega Manuale"
            faseLogName="Piega Manuale"
            faseCGiallaName="Piega"
            items={fasePiegaManuale}
          />
        ) : currentView === 'faseVerniciatura' ? (
          <Pannelatrice
            articles={articles}
            username={username}
            role={role}
            onUpdate={handleArticleUpdate}
            apiPath="/api/fase-verniciatura"
            title="Reparto Verniciatura"
            machineName="Verniciatura"
            faseLogName="Verniciatura"
            faseCGiallaName="Verniciatura"
            items={faseVerniciatura}
          />
        ) : (
          <MovementsView />
        )}
