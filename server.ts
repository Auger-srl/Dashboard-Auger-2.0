  ...
  setInterval(() => {
    try {
      ...
      if (hour === 23 && minute === 0 && lastFaseTaglioArchiveDate !== currentDateStr) {
        ...
        console.log(`[${new Date().toISOString()}] Running automatic archiving for production phases...`);
        
        const tablesToArchive = ['fase_taglio', 'fase_saldatura', 'fase_pannelatrice', 'fase_piega_manuale'];
        
        for (const table of tablesToArchive) {
          try {
            const toArchive = db.prepare(`SELECT * FROM ${table} WHERE fatto = 1 AND stampato = 1`).all() as any[];
            ...
            if (toArchive.length > 0) {
              ...
              console.log(`[${new Date().toISOString()}] Successfully archived ${toArchive.length} rows from ${table}.`);
            }
          } catch (e) {
            console.error(`Error archiving table ${table}:`, e);
          }
        }
      }
    } catch (error) {
...
async function startServer() {
  ...
  // Combined data endpoint to reduce requests and avoid rate limits
  app.get('/api/dashboard-data', (req, res) => {
    try {
      ...
      const macchina5000 = db.prepare('SELECT * FROM macchina_5000 ORDER BY created_at DESC').all();
      const taglioLaser = db.prepare('SELECT * FROM taglio_laser ORDER BY created_at DESC').all();
      const faseTaglio = db.prepare('SELECT * FROM fase_taglio ORDER BY created_at DESC').all();
      const faseSaldatura = db.prepare('SELECT * FROM fase_saldatura ORDER BY created_at DESC').all();
      const fasePannelatrice = db.prepare('SELECT * FROM fase_pannelatrice ORDER BY created_at DESC').all();
      const fasePiegaManuale = db.prepare('SELECT * FROM fase_piega_manuale ORDER BY created_at DESC').all();
      const faseVerniciatura = db.prepare('SELECT * FROM fase_verniciatura ORDER BY created_at DESC').all();
      const productionAlerts = db.prepare('SELECT * FROM production_alerts ORDER BY created_at DESC').all();
      ...
      console.error('Error in combined dashboard data:', error);
  ...
  app.post('/api/macchina-5000', (req, res) => {
    const { data, articolo, quantita, preparazione = 0, inizio = null, fine = null, totale_tempo = null, odl = null, stato = 'da tagliare', operatore = null, cliente = null, commessa = null, note = null } = req.body;
    try {
      const stmt = db.prepare(`
        INSERT INTO macchina_5000 (data, articolo, quantita, preparazione, inizio, fine, totale_tempo, odl, stato, operatore, cliente, commessa, note)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
      const info = stmt.run(data, articolo, quantita, preparazione, inizio, fine, totale_tempo, odl, stato, operatore, cliente, commessa, note);
      res.json({ id: info.lastInsertRowid, data, articolo, quantita, preparazione, inizio, fine, totale_tempo, odl, stato, operatore, cliente, commessa, note });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  });
  ...
  app.post('/api/taglio-laser', (req, res) => {
    ...
    try {
      const stmt = db.prepare(`
        INSERT INTO taglio_laser (data, articolo, quantita, preparazione, inizio, fine, totale_tempo, odl, stato, operatore, cliente, commessa, note)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
      const info = stmt.run(data, articolo, quantita, preparazione, inizio, fine, totale_tempo, odl, stato, operatore, cliente, commessa, note);
      res.json({ id: info.lastInsertRowid, data, articolo, quantita, preparazione, inizio, fine, totale_tempo, odl, stato, operatore, cliente, commessa, note });
  ...
  app.post('/api/fase-taglio', (req, res) => {
    const { lavorazione_per, articolo, quantita, data, fatto = 0, stampato = 0, odl = '', commessa = '', macchina = 'Macchina 5000', note = '' } = req.body;
    try {
      console.log(`[${new Date().toISOString()}] POST /api/fase-taglio - Data:`, { lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, note });
      const stmt = db.prepare('INSERT INTO fase_taglio (lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
      const info = stmt.run(lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, note);
      console.log(`[${new Date().toISOString()}] POST /api/fase-taglio - Success, ID:`, info.lastInsertRowid);
      res.json({ id: info.lastInsertRowid, lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, note });
    } catch (error: any) {
  ...
  app.post('/api/fase-saldatura', (req, res) => {
    const { lavorazione_per, articolo, quantita, data, fatto = 0, stampato = 0, odl = '', commessa = '', macchina = 'Reparto Saldatura', cliente = '', preparazione = 0, inizio = null, pausa = null, inizio2 = null, fine = null, totale_tempo = null, operatore = null, stato = 'da lavorare', note = null } = req.body;
    try {
      const stmt = db.prepare('INSERT INTO fase_saldatura (lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, cliente, preparazione, inizio, pausa, inizio2, fine, totale_tempo, operatore, stato, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
      const info = stmt.run(lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, cliente, preparazione, inizio, pausa, inizio2, fine, totale_tempo, operatore, stato, note);
  ...
  app.put('/api/fase-saldatura/:id', (req, res) => {
    const { id } = req.params;
    const updates = req.body;
    try {
      const existing = db.prepare('SELECT * FROM fase_saldatura WHERE id = ?').get(id) as any;
      if (!existing) return res.status(404).json({ error: 'Record non trovato' });

      const lavorazione_per = updates.lavorazione_per !== undefined ? updates.lavorazione_per : existing.lavorazione_per;
      const articolo = updates.articolo !== undefined ? updates.articolo : existing.articolo;
      const quantita = updates.quantita !== undefined ? updates.quantita : existing.quantita;
      const data = updates.data !== undefined ? updates.data : existing.data;
      const fatto = updates.fatto !== undefined ? updates.fatto : existing.fatto;
      const stampato = updates.stampato !== undefined ? updates.stampato : existing.stampato;
      const odl = updates.odl !== undefined ? updates.odl : existing.odl;
      const commessa = updates.commessa !== undefined ? updates.commessa : existing.commessa;
      const macchina = updates.macchina !== undefined ? updates.macchina : existing.macchina;
      const cliente = updates.cliente !== undefined ? updates.cliente : existing.cliente;
      const preparazione = updates.preparazione !== undefined ? updates.preparazione : existing.preparazione;
      const inizio = updates.inizio !== undefined ? updates.inizio : existing.inizio;
      const pausa = updates.pausa !== undefined ? updates.pausa : existing.pausa;
      ...
      const stmt = db.prepare('UPDATE fase_saldatura SET lavorazione_per = ?, articolo = ?, quantita = ?, data = ?, fatto = ?, stampato = ?, odl = ?, commessa = ?, macchina = ?, cliente = ?, preparazione = ?, inizio = ?, pausa = ?, inizio2 = ?, fine = ?, totale_tempo = ?, operatore = ?, stato = ? WHERE id = ?');
      stmt.run(lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, cliente, preparazione, inizio, pausa, inizio2, fine, totale_tempo, operatore, stato, id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(400).json({ error: error.message });
    }
  ...
  app.delete('/api/fase-saldatura/:id', (req, res) => {
    const { id } = req.params;
    try {
      const stmt = db.prepare('DELETE FROM fase_saldatura WHERE id = ?');
      stmt.run(id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(400).json({ error: error.message });
  ...
  app.put('/api/fase-taglio/:id', (req, res) => {
    const { id } = req.params;
    const { lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, note } = req.body;
    try {
      const stmt = db.prepare('UPDATE fase_taglio SET lavorazione_per = ?, articolo = ?, quantita = ?, data = ?, fatto = ?, stampato = ?, odl = ?, commessa = ?, macchina = ?, note = ? WHERE id = ?');
      stmt.run(lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, note, id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(400).json({ error: error.message });
    }
  ...
  // --- Pannellatrice ---
  app.get('/api/fase-pannelatrice', (req, res) => {
    try {
      const rows = db.prepare('SELECT * FROM fase_pannelatrice ORDER BY created_at DESC').all();
      res.json(rows);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.post('/api/fase-pannelatrice', (req, res) => {
    const { lavorazione_per, articolo, quantita, data, fatto = 0, stampato = 0, odl = '', commessa = '', macchina = 'Pannellatrice', cliente = '', preparazione = 0, inizio = null, pausa = null, inizio2 = null, fine = null, totale_tempo = null, operatore = null, stato = 'da lavorare', note = null } = req.body;
    try {
      const stmt = db.prepare('INSERT INTO fase_pannelatrice (lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, cliente, preparazione, inizio, pausa, inizio2, fine, totale_tempo, operatore, stato, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
      const info = stmt.run(lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, cliente, preparazione, inizio, pausa, inizio2, fine, totale_tempo, operatore, stato, note);
      res.json({ id: info.lastInsertRowid, success: true });
    } catch (error: any) {
      res.status(400).json({ error: error.message });
  ...
  app.put('/api/fase-pannelatrice/:id', (req, res) => {
    const { id } = req.params;
    const updates = req.body;
    try {
      const existing = db.prepare('SELECT * FROM fase_pannelatrice WHERE id = ?').get(id) as any;
      if (!existing) return res.status(404).json({ error: 'Record non trovato' });
      ...
      const stmt = db.prepare('UPDATE fase_pannelatrice SET lavorazione_per = ?, articolo = ?, quantita = ?, data = ?, fatto = ?, stampato = ?, odl = ?, commessa = ?, macchina = ?, cliente = ?, preparazione = ?, inizio = ?, pausa = ?, inizio2 = ?, fine = ?, totale_tempo = ?, operatore = ?, stato = ? WHERE id = ?');
      stmt.run(lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, cliente, preparazione, inizio, pausa, inizio2, fine, totale_tempo, operatore, stato, id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(400).json({ error: error.message });
  ...
  app.delete('/api/fase-pannelatrice/:id', (req, res) => {
    const { id } = req.params;
    try {
      const stmt = db.prepare('DELETE FROM fase_pannelatrice WHERE id = ?');
      stmt.run(id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(400).json({ error: error.message });
  ...
  // --- Piega Manuale ---
  app.get('/api/fase-piega-manuale', (req, res) => {
    try {
      const rows = db.prepare('SELECT * FROM fase_piega_manuale ORDER BY created_at DESC').all();
      res.json(rows);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.post('/api/fase-piega-manuale', (req, res) => {
    const { lavorazione_per, articolo, quantita, data, fatto = 0, stampato = 0, odl = '', commessa = '', macchina = 'Piega Manuale', cliente = '', preparazione = 0, inizio = null, pausa = null, inizio2 = null, fine = null, totale_tempo = null, operatore = null, stato = 'da lavorare', note = null } = req.body;
    try {
      const stmt = db.prepare('INSERT INTO fase_piega_manuale (lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, cliente, preparazione, inizio, pausa, inizio2, fine, totale_tempo, operatore, stato, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
      const info = stmt.run(lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, cliente, preparazione, inizio, pausa, inizio2, fine, totale_tempo, operatore, stato, note);
      res.json({ id: info.lastInsertRowid, success: true });
    } catch (error: any) {
      res.status(400).json({ error: error.message });
  ...
  app.put('/api/fase-piega-manuale/:id', (req, res) => {
    const { id } = req.params;
    const updates = req.body;
    try {
      const existing = db.prepare('SELECT * FROM fase_piega_manuale WHERE id = ?').get(id) as any;
      if (!existing) return res.status(404).json({ error: 'Record non trovato' });
      ...
      const stmt = db.prepare('UPDATE fase_piega_manuale SET lavorazione_per = ?, articolo = ?, quantita = ?, data = ?, fatto = ?, stampato = ?, odl = ?, commessa = ?, macchina = ?, cliente = ?, preparazione = ?, inizio = ?, pausa = ?, inizio2 = ?, fine = ?, totale_tempo = ?, operatore = ?, stato = ? WHERE id = ?');
      stmt.run(lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, cliente, preparazione, inizio, pausa, inizio2, fine, totale_tempo, operatore, stato, id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(400).json({ error: error.message });
  ...
  app.delete('/api/fase-piega-manuale/:id', (req, res) => {
    const { id } = req.params;
    try {
      const stmt = db.prepare('DELETE FROM fase_piega_manuale WHERE id = ?');
      stmt.run(id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(400).json({ error: error.message });
  ...
  app.get('/api/fase-verniciatura', (req, res) => {
    try {
      const rows = db.prepare('SELECT * FROM fase_verniciatura ORDER BY created_at DESC').all();
      res.json(rows);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.post('/api/fase-verniciatura', (req, res) => {
    const { lavorazione_per, articolo, quantita, data, fatto = 0, stampato = 0, odl = '', commessa = '', macchina = 'Verniciatura', cliente = '', stato = 'da lavorare', note = null } = req.body;
    try {
      const stmt = db.prepare('INSERT INTO fase_verniciatura (lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, cliente, stato, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
      const info = stmt.run(lavorazione_per, articolo, quantita, data, fatto, stampato, odl, commessa, macchina, cliente, stato, note);
      res.json({ id: info.lastInsertRowid, success: true });
    } catch (error: any) {
      res.status(400).json({ error: error.message });
    }
  ...
  app.put('/api/fase-verniciatura/:id', (req, res) => {
    const { id } = req.params;
    const updates = req.body;
    try {
      const existing = db.prepare('SELECT * FROM fase_verniciatura WHERE id = ?').get(id) as any;
      if (!existing) return res.status(404).json({ error: 'Record non trovato' });

      const attributes = [
        'lavorazione_per', 'articolo', 'quantita', 'data', 'fatto', 'stampato',
        'odl', 'commessa', 'macchina', 'cliente', 'stato',
        'preparazione', 'inizio', 'inizio2', 'pausa', 'fine', 'totale_tempo', 'operatore'
      ];

      const values = attributes.map(attr => updates[attr] !== undefined ? updates[attr] : existing[attr]);

      const placeholders = attributes.map(attr => `${attr} = ?`).join(', ');
      
      const stmt = db.prepare(`UPDATE fase_verniciatura SET ${placeholders} WHERE id = ?`);
      stmt.run(...values, id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(400).json({ error: error.message });
  ...
  app.delete('/api/fase-verniciatura/:id', (req, res) => {
    const { id } = req.params;
    try {
      const stmt = db.prepare('DELETE FROM fase_verniciatura WHERE id = ?');
      stmt.run(id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(400).json({ error: error.message });
  ...
  app.post('/api/archivio-stampe', (req, res) => {
    const { lavorazione_per, articolo, quantita, data, odl = '', commessa = '', macchina = 'Macchina 5000', note = '' } = req.body;
    try {
      const stmt = db.prepare('INSERT INTO archivio_stampe (lavorazione_per, articolo, quantita, data, odl, commessa, macchina, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
      const info = stmt.run(lavorazione_per, articolo, quantita, data, odl, commessa, macchina, note);
      res.json({ id: info.lastInsertRowid, success: true });
    } catch (error: any) {
      res.status(400).json({ error: error.message });
    }
  ...
  app.post('/api/programmi-eseguiti', (req, res) => {
    const { lavorazione_per, articolo, quantita, data, odl = '', commessa = '', macchina = 'Macchina 5000', note = '' } = req.body;
    try {
      const stmt = db.prepare('INSERT INTO programmi_eseguiti (lavorazione_per, articolo, quantita, data, odl, commessa, macchina, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
      const info = stmt.run(lavorazione_per, articolo, quantita, data, odl, commessa, macchina, note);
      res.json({ id: info.lastInsertRowid, success: true });
    } catch (error: any) {
      res.status(400).json({ error: error.message });
      ...
      const transaction = db.transaction(() => {
        const executeMovement = (id: string | number, isAuto: boolean = false, overrideTipo?: string, overrideFase?: string, overrideQty?: number) => {
          ...
          if (tipo === 'carico' || tipo === 'CARICO COMP. AGM' || tipo === 'CARICO COMP. AGR') {
              if (fase === 'taglio' || fase === 'Tagliato - Carico da Macchina 5000' || fase === 'Tagliato - Carico da Taglio Laser') {
                  taglio += currentQty;
              } else if (fase === 'piega' || fase.includes('Pannellatura') || fase.includes('Piega Manuale')) {
                  piega += currentQty;
                  if (taglioEnabled && tipo === 'carico') {
                      taglio -= currentQty; // Grezzo -> Tagliato
                      prevFase = 'taglio';
                  }
              } else if (fase === 'saldatura' || fase.includes('Saldatura')) {
              ...
              } else if (fase === 'verniciatura' || fase.includes('Verniciatura')) {
              ...
              if (fase === 'taglio' || fase.includes('Tagliato')) { prevVal = taglio; taglio = currentQty; }
              else if (fase === 'piega' || fase.includes('Pannellatura') || fase.includes('Piega Manuale')) { prevVal = piega; piega = currentQty; }
              else if (fase === 'saldatura' || fase.includes('Saldatura')) { prevVal = saldatura; saldatura = currentQty; }
              else if (fase === 'verniciatura' || fase.includes('Verniciatura')) { prevVal = verniciatura; verniciatura = currentQty; verniciati = currentQty; }
