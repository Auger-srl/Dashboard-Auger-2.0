...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS fase_saldatura (
    ...
    stato TEXT DEFAULT 'da lavorare',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
  );

  CREATE TABLE IF NOT EXISTS fase_pannelatrice (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    lavorazione_per TEXT,
    articolo TEXT,
    quantita INTEGER,
    ...
    fatto INTEGER DEFAULT 0,
    stampato INTEGER DEFAULT 0,
    macchina TEXT DEFAULT 'Pannellatrice',
    stato TEXT DEFAULT 'da lavorare',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
  );

  CREATE TABLE IF NOT EXISTS fase_piega_manuale (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    lavorazione_per TEXT,
    articolo TEXT,
    quantita INTEGER,
    ...
    fatto INTEGER DEFAULT 0,
    stampato INTEGER DEFAULT 0,
    macchina TEXT DEFAULT 'Piega Manuale',
    stato TEXT DEFAULT 'da lavorare',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
  );

  CREATE TABLE IF NOT EXISTS fase_verniciatura (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    lavorazione_per TEXT,
    articolo TEXT,
    quantita INTEGER,
  ...
  CREATE INDEX IF NOT EXISTS idx_fase_pannelatrice_articolo ON fase_pannelatrice(articolo);
  CREATE INDEX IF NOT EXISTS idx_fase_piega_manuale_articolo ON fase_piega_manuale(articolo);
...
// Migrations for new columns in phase tables
const tablesToUpdate = ['fase_saldatura', 'fase_pannelatrice', 'fase_piega_manuale', 'fase_verniciatura', 'macchina_5000', 'taglio_laser'];
const columnsToAdd = [
  { name: 'cliente', type: 'TEXT' },
  { name: 'commessa', type: 'TEXT' },
  { name: 'preparazione', type: 'INTEGER DEFAULT 0' },
  { name: 'inizio', type: 'TEXT' },
  { name: 'inizio2', type: 'TEXT' },
  { name: 'pausa', type: 'TEXT' },
  { name: 'fine', type: 'TEXT' },
  { name: 'totale_tempo', type: 'INTEGER' },
  { name: 'operatore', type: 'TEXT' },
  { name: 'stato', type: "TEXT DEFAULT 'da lavorare'" },
  { name: 'note', type: 'TEXT' }
];
...
try {
  db.exec('ALTER TABLE fase_taglio ADD COLUMN note TEXT;');
} catch(e) {}
try {
  db.exec('ALTER TABLE archivio_stampe ADD COLUMN note TEXT;');
} catch(e) {}
try {
  db.exec('ALTER TABLE programmi_eseguiti ADD COLUMN macchina TEXT;');
} catch(e) {}
try {
  db.exec('ALTER TABLE programmi_eseguiti ADD COLUMN note TEXT;');
} catch(e) {}
